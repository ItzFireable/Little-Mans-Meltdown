...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			...
			case 'geno':
				tex = Paths.getSparrowAtlas('characters/geno_assets');
				frames = tex;
				animation.addByPrefix('idle', 'geno idle dance', 24);
				animation.addByPrefix('singUP', 'geno note up', 24);
				animation.addByPrefix('singRIGHT', 'geno note right', 24);
				animation.addByPrefix('singDOWN', 'geno note down', 24);
				animation.addByPrefix('singLEFT', 'geno note left', 24);

				addOffset('idle');
				addOffset("singUP", -25, 7);
				addOffset("singRIGHT", -1, 16);
				addOffset("singLEFT", -5, 11);
